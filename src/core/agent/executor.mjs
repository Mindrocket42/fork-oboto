...
import { PipelineEngine } from './pipeline.mjs';
import { VFSSyncAdapter } from './vfs.mjs';
import { selfList, selfRead, selfWrite, selfRestart } from './self-awareness.mjs';
...
export const executeCommand = async (commandStr, vfs, voluntaryMem, involMem) => {
    ...
    if (['eval', 'finish', 'mem_store', 'mem_search', 'mem_retract', 'mem_supersede', 'mutate', 'spawn', 'wait', 'self_restart'].includes(cmd)) {
    ...
    try {
        switch (cmd) {
            ...
            case 'mem_search':
                ...
                return results.length === 0 ? { result: "No relevant memories found." } : { result: results.map(r => `[ID: ${r.id}] [Score: ${r.score.toFixed(2)}] [${r.validity || 'valid'}] ${r.text}`).join('\n') };
            case 'mem_retract': {
                // mem_retract <id> [reason]
                const retractId = parseInt(args[1], 10);
                if (isNaN(retractId)) return { error: "Invalid ID. Usage: mem_retract <id> [reason]" };
                const reason = args.slice(2).join(' ') || 'Manually retracted';
                const retracted = voluntaryMem.retract(retractId, reason);
                return retracted
                  ? { result: `Memory ID ${retractId} retracted. Reason: ${reason}` }
                  : { error: `Memory ID ${retractId} not found.` };
            }
            case 'mem_supersede': {
                // mem_supersede <old_id> <new_text>
                const oldId = parseInt(args[1], 10);
                if (isNaN(oldId)) return { error: "Invalid ID. Usage: mem_supersede <old_id> <new_text>" };
                const newText = commandStr.substring(commandStr.indexOf(args[1]) + args[1].length).trim();
                if (!newText) return { error: "Missing new text. Usage: mem_supersede <old_id> <new_text>" };
                const newId = await voluntaryMem.supersede(oldId, newText);
                return newId != null
                  ? { result: `Memory ID ${oldId} superseded by new entry ID ${newId}.` }
                  : { error: `Memory ID ${oldId} not found.` };
            }
            case 'mutate':
